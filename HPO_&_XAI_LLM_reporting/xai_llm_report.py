...
# ====== CONFIG (environment-variable based for GitHub safety) ======
CKPT_PATH = os.getenv("CKPT_PATH", "./models/best.pt")  
SPEC_DIR  = os.getenv("SPEC_DIR", "./spectrograms")      
PATIENT   = os.getenv("PATIENT", "chb01")
OUT_DIR   = os.getenv("OUT_DIR", "./xai_report")         
...
REQUIRE_LLM = bool(int(os.getenv("REQUIRE_LLM", "1"))) # do not skip; fail the run if LLM can't be called successfully
LLM_TOTAL_TIMEOUT_S = int(os.getenv("LLM_TOTAL_TIMEOUT_S", "300")) # hard cap on total retry time (e.g., 15 minutes)
LLM_MAX_RETRIES = int(os.getenv("LLM_MAX_RETRIES", "10")) # safety cap on attempts (very high)
LLM_INITIAL_BACKOFF_S = float(os.getenv("LLM_INITIAL_BACKOFF_S", "2.0")) # starting backoff (exponential)
LLM_BACKOFF_CAP_S = float(os.getenv("LLM_BACKOFF_CAP_S", "60.0")) # max backoff between attempts
LLM_TIMEOUT_PER_CALL_S = int(os.getenv("LLM_TIMEOUT_PER_CALL_S", "90")) # HTTP timeout per request
# ============================================================================================
